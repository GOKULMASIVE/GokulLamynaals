...
const AddReceivableConfig = (props) => {
  ...
  const onSubmit = (e) => {
    ...
    if (e.CC && e.GVW && e.year) {
      if (
        ...
        const ApiType =
          formType === "edit"
            ? UpdateReceivableConfig(e)
            : PostReceivableConfig(e);
        ApiType.then((res) => {
          ActiveFunction();
          setOpenDrawer(false);
        }).catch((err) => {
    ...
    <>
      <div>
        <Grid container className="DrawerHeader">
          <Grid item xs={6} sm={6}>
            <Typography>Add Receivable Configuration</Typography>
          </Grid>
                        ...
                        <Select
                          ...
                          onChange={(selectedValues) => {
                            try {
                              ...
                              const bookingCodeIds = parsedObjects
                                .filter((el) => el.bookingCodeId !== undefined)
                                .map((el) => el.bookingCodeId);
                              const SubbookingCodeIds = parsedObjects
...
export default AddReceivableConfig;
