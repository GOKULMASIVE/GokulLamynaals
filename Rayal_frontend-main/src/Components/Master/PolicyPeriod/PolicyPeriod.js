...
import AddPolicyPeriod from "./AddPolicyPeriod";
...
import { GetPolicyPeriod, DeletePolicyPeriod, UpdatePolicyPeriod, FilterPolicyPeriod } from "../../../Service/_index";
...
const PolicyPeriod = () => {
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(filter.fn(policyPeriod), getComparator(order, orderBy))?.slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, filter, policyPeriod]
  );
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - policyPeriod.length) : 0;

  const GetData = () => {
    GetPolicyPeriod().then((res) => {
      console.log(res)
      setPolicyPeriod(res?.data);
    });
  };
  ...
  const GetActiveData = () => {
    GetPolicyPeriod().then((res) => {
      setPolicyPeriod(res?.data?.filter((item) => item.isEnabled === true));
    });
  };
  const GetInActiveData = () => {
    GetPolicyPeriod().then((res) => {
      setPolicyPeriod(res?.data?.filter((item) => item.isEnabled === false));
    });
  };
  React.useEffect(() => {
    GetActiveData();
  ...
  const EditPolicyPeriodData = (row) => {
    setSelectedData(row);
    setOpenDrawer(true);
    setFormType("edit")
  ...
  const ViewPolicyPeriodData = (row) => {
    setSelectedData(row);
    setOpenDrawer(true);
    setFormType("view")
  }

  const ActivePolicyPeriodData = (row, active) => {
  ...
  const ActiveFunction = () => {
    UpdatePolicyPeriod(selectedData._id, { isEnabled: activeBoolean }).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const DeletePolicyPeriodData = (row) => {
    setOpenDeleteConfirmBox(true);
    setSelectedData(row);
  };
  const DeleteFunction = () => {
    DeletePolicyPeriod(selectedData._id).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const FilterPolicyPeriodFunction = (e) => {
    setPageView(e?.value)
    let data = e?.isEnabled;
    let apiMethod =
      (data === "all")
        ? GetData()
        : FilterPolicyPeriod({ isEnabled: data }).then((res) => {
          setPolicyPeriod(res?.data);
        });
  };
  return (
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Policy Period</Typography>
          </Grid>
          ...
          <Grid item xs={12} sm={2} >
            <Autocomplete
              ...
              onChange={(option, value) => { FilterPolicyPeriodFunction(value)}}
              clearIcon={false}
          ...
          <Grid item xs={12} sm={1} className="d-flex justify-content-end">
            <Button
              ...
              endIcon={<AddIcon />}
              onClick={() => CreatePolicyPeriodData()}
            >
              Create
            </Button>
          </Grid>
      ...
      <div className="d-flex PageContainer">
        <Paper className= "container-fluid TableBox">
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Box className="ActionIcons">
                          <Tooltip title="edit">
                            <EditIcon onClick={() => EditPolicyPeriodData(row)} />
                          </Tooltip>
                          <Tooltip title="view">
                            <RemoveRedEyeIcon onClick={() => ViewPolicyPeriodData(row)} />
                          </Tooltip>
                          <Tooltip title="Active">
                            {row.isEnabled ? (
                              <Switch
                                onClick={() =>
                                  ActivePolicyPeriodData(row, row.isEnabled)
                                }
                                checked={true}
                                color="warning"
                              />
                          ...
                          <Tooltip title="delete">
                            <DeleteIcon
                              onClick={() => DeletePolicyPeriodData(row)}
                            />
                          </Tooltip>
                        </Box>
                      </TableCell>
          ...
          <TablePagination
            ...
            component="div"
            count={filter.fn(policyPeriod).length || 0}
