...
import AddLinkBookingCode from "./AddLinkBookingCode";
...
import { GetLinkBookingCode, DeleteLinkBookingCode, FilterLinkBookingCode, UpdateLinkBookingCode } from "../../../Service/_index";
...
const LinkBookingCode = () => {
  ...
  const [linkBookingCode, setLinkBookingCode] = React.useState([]);
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(filter.fn(linkBookingCode), getComparator(order, orderBy))?.slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, filter, linkBookingCode]
  );
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - linkBookingCode.length) : 0;


  const GetData = () => {
    GetLinkBookingCode().then((res) => {
      console.log(res)
      setLinkBookingCode(res.data);
    });
  };
  ...
  const GetActiveData = () => {
    GetLinkBookingCode().then((res) => {
      setLinkBookingCode(res?.data?.filter((item) => item.isEnabled === true));
    });
  };
  const GetInActiveData = () => {
    GetLinkBookingCode().then((res) => {
      setLinkBookingCode(res?.data?.filter((item) => item.isEnabled === false));
    });
  };
  ...
  const CreateLinkBookingCodeData = () => {
    setSelectedData({});
    setOpenDrawer(true);
    setFormType("add")
  ...
  const DeleteLinkBookingCodeData = (row) => {
    setOpenDeleteConfirmBox(true);
    setSelectedData(row);
  };

  const ViewLinkBookingCode = (row) => {
    setSelectedData(row);
    setOpenDrawer(true);
    setFormType("view")
  }
  const DeleteFunction = () => {
    DeleteLinkBookingCode(selectedData._id).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const ActiveFunction = () => {
    UpdateLinkBookingCode(selectedData._id, { isEnabled: activeBoolean }).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
      ...
      fn: (items) => {
        if (target.value == "") return items;
          ...
          return items.filter((el) =>
            `${el?.companyId?.shortName},${el?.bookingCodeId?.bookingCode},${el.remarks}`
              .toLowerCase()
  ...
  const FilterLinkBookingCodeFunction = (e) => {
    setPageView(e?.value)
    let data = e?.isEnabled;
    let apiMethod =
      (data === "all")
        ? GetData()
        : FilterLinkBookingCode({ isEnabled: data }).then((res) => {
          setLinkBookingCode(res?.data);
        });
  };
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Link Booking Code</Typography>
          </Grid>
          ...
          <Grid item xs={12} sm={2} >
            <Autocomplete
              ...
              onChange={(option, value) => {
                FilterLinkBookingCodeFunction(value);
              }}
              clearIcon={false}
          ...
          <Grid item xs={12} sm={1} className="d-flex justify-content-end">
            <Button
              ...
              endIcon={<AddIcon />}
              onClick={() => CreateLinkBookingCodeData()}
            >
              Create
            </Button>
          </Grid>
      ...
      <div className="d-flex PageContainer">
        ...
        >
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Box className="ActionIcons">
                          <Tooltip title="view">
                            <RemoveRedEyeIcon onClick={() => ViewLinkBookingCode(row)} />
                          </Tooltip>
                          ...
                          <Tooltip title="delete">
                            <DeleteIcon
                              onClick={() => DeleteLinkBookingCodeData(row)}
                            />
                          </Tooltip>
                        </Box>
                      </TableCell>
          ...
          <TablePagination
            ...
            component="div"
            count={filter.fn(linkBookingCode).length || 0}
...
export default LinkBookingCode;
