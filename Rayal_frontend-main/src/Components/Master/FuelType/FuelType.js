...
import { GetFuelType, UpdateFuelType, FilterFuelType } from "../../../Service/_index";
...
const FuelType = () => {
  ...
  const [openFuelTypeDrawer, setOpenFuelTypeDrawer] = React.useState(false);
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(filter.fn(fuelType), getComparator(order, orderBy))?.slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, filter, fuelType]
  );
  ...
  const GetActiveData = () => {
    GetFuelType().then((res) => {
      setFuelType(res.data.filter((item) => item.isEnabled === true));
    });
  };
  const GetInActiveData = () => {
    GetFuelType().then((res) => {
      setFuelType(res.data.filter((item) => item.isEnabled === false));
    });
  };
  ...
  const EditFuelTypeData = (row) => {
    setSelectedData(row);
    setOpenFuelTypeDrawer(true);
  };

  const CreateFuelTypeData = () => {
    setSelectedData({});
    setOpenFuelTypeDrawer(true);
  };

  const ActiveFuelTypeData = (row, active) => {
    setOpenConfirmBox(true);
    setSelectedData(row);
    setActiveBoolean(!active);
  };
  const ActiveFunction = (event) => {
    UpdateFuelType(selectedData._id, { isEnabled: activeBoolean }).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const FilterFuelTypeFunction = (e) => {
    setPageView(e?.value)
    let data = e?.isEnabled;
    let apiMethod =
      data === "all"
        ? GetData()
        : FilterFuelType({ isEnabled: data }).then((res) => {
          setFuelType(res?.data);
        });
  };
  return (
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Fuel Type</Typography>
          </Grid>
      ...
      <div className="d-flex PageContainer">
        <Paper className="container-fluid TableBox ">
                    ...
                    <TableRow hover tabIndex={-1} key={index}>
                      ...
                      <TableCell align="center">
                        <Box className="ActionIcons">
                          <Tooltip title="edit">
                            <EditIcon onClick={() => EditFuelTypeData(row)} />
                          </Tooltip>
                          ...
                          <Tooltip title="Active">
                            {row.isEnabled ? (
                              <Switch
                                onClick={() =>
                                  ActiveFuelTypeData(row, row.isEnabled)
                                }
                                checked={true}
                                color="warning"
                              />
          ...
          <TablePagination
            ...
            component="div"
            count={filter.fn(fuelType).length || 0}
      ...
      >
        {openFuelTypeDrawer ? (
          <AddFuelType
            ...
            setOpenFuelTypeDrawer={setOpenFuelTypeDrawer}
