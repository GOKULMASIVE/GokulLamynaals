...
import { GetProduct, DeleteProduct, UpdateProduct, FilterProduct } from "../../../Service/_index";
...
const Product = () => {
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(filter.fn(product), getComparator(order, orderBy))?.slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, filter, product]
  );
  ...
  const GetData = () => {
    GetProduct({ isAscending: false }).then((res) => {
      setProduct(res?.data);
    });
  };
  ...
  const GetActiveData = () => {
    GetProduct({ isAscending: false }).then((res) => {
      setProduct(res?.data?.filter((item) => item.isEnabled === true));
    });
  };
  const GetInActiveData = () => {
    GetProduct({ isAscending: false }).then((res) => {
      setProduct(res?.data?.filter((item) => item.isEnabled === false));
    });
  };
  ...
  const ActiveFunction = () => {
    UpdateProduct(selectedData._id, { isEnabled: activeBoolean }).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const DeleteFunction = () => {
    DeleteProduct(selectedData._id).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const FilterProductFunction = (e) => {
    ...
    let apiMethod =
      (data === "all")
        ? GetData()
        : FilterProduct({ isEnabled: data }).then((res) => {
          setProduct(res?.data);
        });
  };
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Product</Typography>
          </Grid>
      ...
      <div className="d-flex PageContainer">
        <Paper className="container-fluid TableBox">
                      ...
                      >
                        ...
                        <TableCell align="center">
                          <Box className="ActionIcons">
                            ...
                            <Tooltip title="view">
                              <RemoveRedEyeIcon onClick={() => ViewProductData(row)} />
                            </Tooltip>
            ...
            <TablePagination
              ...
              component="div"
              count={filter.fn(product).length || 0}
