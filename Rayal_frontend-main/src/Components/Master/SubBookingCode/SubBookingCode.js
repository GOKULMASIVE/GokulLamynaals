...
import AddSubBookingCode from "./AddSubBookingCode";
...
const SubBookingCode = () => {
  ...
  const [subBookingCode, setSubBookingCode] = React.useState([]);
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(filter.fn(subBookingCode), getComparator(order, orderBy))?.slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, filter, subBookingCode]
  );
  ...
  const GetData = () => {
    GetSubBookingCode({ isAscending: false }).then((res) => {
      setSubBookingCode(res.data);
    });
  };

  const GetActiveData = () => {
    GetSubBookingCode({ isAscending: false }).then((res) => {
      const data = res?.data?.filter((item) => item.isEnabled === true);
      setSubBookingCode(data);
    });
  };
  const GetInActiveData = () => {
    GetSubBookingCode({ isAscending: false }).then((res) => {
      setSubBookingCode(res?.data?.filter((item) => item.isEnabled === false));
    });
  };
  ...
  const EditSubBookingCodeData = (row) => {
    setSelectedData(row);
    setOpenDrawer(true);
    setFormType("edit");
  };

  const CreateSubBookingCodeData = () => {
    setSelectedData({});
    setOpenDrawer(true);
    setFormType("add");
  };
  const ViewSubBookingCodeData = (row) => {
    setSelectedData(row);
    setOpenDrawer(true);
    setFormType("view");
  };

  const ActiveSubBookingCodeData = (row, active) => {
  ...
  const ActiveFunction = () => {
    UpdateSubBookingCode(selectedData._id, { isEnabled: activeBoolean }).then(
      (res) => {
        switch (pageView) {
          case 1:
            GetData();
  ...
  const DeleteSubBookingCodeData = (row) => {
    setOpenDeleteConfirmBox(true);
    setSelectedData(row);
  };
  const DeleteFunction = () => {
    DeleteSubBookingCode(selectedData._id).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
      ...
      fn: (items) => {
        if (target.value == "") return items;
          ...
          return items.filter((el) =>
            `${el?.subBookingCode}${el?.remarks} , ${el?.bookingCode}`
              .toLowerCase()
  ...
  const FilterSubBookingCodeFunction = (e) => {
    setPageView(e?.value);
    let data = e?.isEnabled;
    let apiMethod =
      data === "all"
        ? GetData()
        : FilterSubBookingCode({ isEnabled: data }).then((res) => {
          setSubBookingCode(res?.data);
        });
  };
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Sub Booking Code</Typography>
          </Grid>
          ...
          <Grid item xs={12} sm={2}>
            <Autocomplete
              ...
              onChange={(option, value) => {
                FilterSubBookingCodeFunction(value);
              }}
              clearIcon={false}
          ...
          <Grid item xs={12} sm={1} className="d-flex justify-content-end">
            <Button
              ...
              endIcon={<AddIcon />}
              onClick={() => CreateSubBookingCodeData()}
            >
              Create
            </Button>
          </Grid>
      ...
      <div className="d-flex PageContainer">
        <Paper className={"container-fluid TableBox "}>
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Box className="ActionIcons">
                          <Tooltip title="edit">
                            <EditIcon
                              onClick={() => EditSubBookingCodeData(row)}
                            />
                          </Tooltip>
                          <Tooltip title="view">
                            <RemoveRedEyeIcon
                              onClick={() => ViewSubBookingCodeData(row)}
                            />
                          </Tooltip>
                          <Tooltip title="Active">
                            {row.isEnabled ? (
                              <Switch
                                onClick={() =>
                                  ActiveSubBookingCodeData(row, row.isEnabled)
                                }
                                checked={true}
                                color="warning"
                              />
                          ...
                          <Tooltip title="delete">
                            <DeleteIcon
                              onClick={() => DeleteSubBookingCodeData(row)}
                            />
                          </Tooltip>
                        </Box>
                      </TableCell>
          ...
          <TablePagination
            ...
            component="div"
            count={filter.fn(subBookingCode)?.length || 0}
