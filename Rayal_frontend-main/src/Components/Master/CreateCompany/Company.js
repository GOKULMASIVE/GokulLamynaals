...
const Company = () => {
  ...
  const visibleRows = React.useMemo(
    () =>
      stableSort(filter.fn(company), getComparator(order, orderBy))?.slice(
        page * rowsPerPage,
        page * rowsPerPage + rowsPerPage
      ),
    [order, orderBy, page, rowsPerPage, filter, company]
  );
  ...
  const GetData = () => {
    GetCompany({ isAscending: false }).then((res) => {
      setCompany(res?.data);
    });
  };
  ...
  const GetActiveData = () => {
    GetCompany({ isAscending: false }).then((res) => {
      setCompany(res?.data.filter((item) => item.isEnabled === true));
    });
  };
  const GetInActiveData = () => {
    GetCompany({ isAscending: false }).then((res) => {
      setCompany(res?.data.filter((item) => item.isEnabled === false));
    });
  };
  ...
  const ActiveCompanyData = (row, active) => {
  ...
  const ActiveFunction = () => {
    UpdateCompany(selectedData._id, { isEnabled: activeBoolean }).then(
      () => {
        switch (pageView) {
          case 1:
            GetData();
  ...
  const DeleteCompanyData = (row) => {
    setOpenDeleteConfirmBox(true);
    setSelectedData(row);
  };

  const DeleteFunction = () => {
    DeleteCompany(selectedData._id).then(() => {
      switch (pageView) {
        case 1:
          GetData();
          break;
      ...
      fn: (items) => {
        if (target.value == "") return items;
          ...
          return items.filter((el) =>
            `${el?.companyName},${el?.remarks},${el?.shortName}`
              .toLowerCase()
  ...
  const FilterCompanyFunction = (e) => {
    setPageView(e?.value);
    let data = e?.isEnabled;
    let apiMethod =
      data === "all"
        ? GetData()
        : FilterCompany({ isEnabled: data }).then((res) => {
          setCompany(res?.data);
        });
  };
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Insurance Company</Typography>
          </Grid>
          ...
          <Grid item xs={12} sm={2}>
            <Autocomplete
              ...
              onChange={(option, value) => {FilterCompanyFunction(value)}}
              clearIcon={false}
      ...
      <div className="d-flex PageContainer">
        <Paper className="container-fluid TableBox ">
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Box className="ActionIcons">
                          <Tooltip title="edit">
                            <EditIcon onClick={() => EditCompanyData(row)} />
                          </Tooltip>
                          <Tooltip title="view">
                            <RemoveRedEyeIcon
                              onClick={() => ViewCompanyData(row)}
                            />
                          </Tooltip>
                          <Tooltip title="Active">
                            {row.isEnabled ? (
                              <Switch
                                onClick={() =>
                                  ActiveCompanyData(row, row.isEnabled)
                                }
                                checked={true}
                                color="warning"
                                sx={{ color: "green" }}
          ...
          <TablePagination
            ...
            component="div"
            count={filter.fn(company)?.length || 0} 
