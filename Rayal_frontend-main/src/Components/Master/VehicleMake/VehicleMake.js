...
import { GetVehicleMake, UpdateVehicleMake, FilterVehicleMake, DeleteVehicleMake,PostVehicleMake } from "../../../Service/_index";
...
//   GetVehicleMake().then((res)=>console.log(res.data))
// }
// useEffect(()=>{
...
const VehicleMake = () => {
  ...
  const ActiveFunction = (event) => {
    UpdateVehicleMake(selectedData._id, { isEnabled: activeBoolean }).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
  ...
  const DeleteVehicleMakeData = (row) => {
    setOpenDeleteConfirmBox(true);
    setSelectedData(row);
  };
  const DeleteFunction = () => {
    DeleteVehicleMake(selectedData._id).then((res) => {
      switch (pageView) {
        case 1:
          GetData();
          break;
      ...
      fn: (items) => {
        if (target.value == "") return items;
          ...
          return items.filter((el) =>
            `${el.product},${el.subProduct},${el.remarks},${el.vehicleMake}`
              .toLowerCase()
  ...
  const FilterVehicleMakeFunction = (e) => {
    setPageView(e?.value)
    let data = e?.isEnabled;
    let apiMethod =
      data === "all"
        ? GetData()
        : FilterVehicleMake({ isEnabled: data }).then((res) => {
          setBranch(res?.data);
        });
  };
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid container className="Master_Header_Container" spacing={2}>
          <Grid item xs={12} sm={3}>
            <Typography className="Master_Header_Heading">Vehicle Make</Typography>
          </Grid>
          ...
          <Grid item xs={12} sm={2} >
            <Autocomplete
              ...
              onChange={(option, value) => {
                FilterVehicleMakeFunction(value);
              }}
              clearIcon={false}
                ...
                {visibleRows?.map((row, index) => {
                  ...
                  return (
                    <TableRow hover tabIndex={-1} key={index}>
                      ...
                      <TableCell align="left">{row.vehicleMake}</TableCell>
                      ...
                      <TableCell align="center">
                        <Box className="ActionIcons">
                          ...
                          <Tooltip title="delete">
                            <DeleteIcon
                              onClick={() => DeleteVehicleMakeData(row)}
                            />
                          </Tooltip>
                        </Box>
                      </TableCell>
