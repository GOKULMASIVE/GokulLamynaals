...
const EditPolicyList = (props) => {

  let formRef = useRef();
  const { setOpenEditPolicyDrawer, selectedData, formType, GetData, editType, openEditPolictDrawer } = props;
  ...
  const [vehicleValue, setVehicleValue] = useState(formType === "edit" ? selectedData?.make?.vehicleMake : null);
  ...
  const GetSubProductDetails = () => {
    GetSubProduct({ isAscending: true }).then((res) => {
      ...
      const modifiedSubProductDetails = res.data
        ...
        .filter(Boolean);
        console.log("modifiedSubProductDetails:",modifiedSubProductDetails);
      const FilterSubProductDetails = modifiedSubProductDetails.filter((item) => item?.productID === productIdValue)
      console.log("FilterData:",FilterSubProductDetails);
  ...
  const onSubmit = (data) => {
    ...
    if (true) {
      ...
      //       editType === "CommonEdit" && setOpenEditPolicyDrawer(false);
      //     } else {
      //       setOpenEditPolicyDrawer(true);
      //     }
      //   }
                      ...
                      <TabContext value={value}>
                        ...
                        <TabPanel value={1}>
                          <Grid container sx={{ p: 1 }} spacing={2}>
                            ...
                            <Grid item xs={12} sm={4}>
                              <FloatLabel label="Sub Product"  value={values?.subProductId}>
                                <Autocomplete
                                  className="AutoComplete_InputBox"
                                  name="subProductId"
