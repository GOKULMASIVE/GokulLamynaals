...
const BranchPayable = () => {
    ...
    const ApproveFunction = (row) => {
        ...
        GetBranchpayablePercentage(row._id).then((res) => {
            setOpenLoader(false)
            setOpenDrawer(true);
    ...
    return (
        <>
            <Grid container columnSpacing={2} rowSpacing={1} sx={{ padding: '10px 10px 0 10px' }} className="Master_Header_Container" >
                ...
                <Grid item sm={3} xs={12}>
                    <FloatLabel label="Start date & End Date" value="react">
                        <RangePicker placement='bottomLeft' className="Range_Picker w-100" style={{ borderRadius: '0px' }} onChange={(e) => {
            ...
            <Box sx={{ flexGrow: 1 }} mt={4}>
                <Grid container className="Master_Header_Container" spacing={2}>
                    <Grid item xs={12} sm={3}>
                        <Typography className="Master_Header_Heading">Branch Payable</Typography>
                    </Grid>
                                            ...
                                            <TableCell component="th" scope="row" padding="normal">
                                                {
                                                    row.filterType === "APPROVE_DATE" ?
                                                        <>{moment(row?.branchPayable?.createdAt).format("DD-MM-YYYY LT")}</> : <>{moment(row?.issueDate).format("D-M-Y")}</>
                                                }
                                            </TableCell>
                                            <TableCell align="left">{row?.companyId?.shortName}</TableCell>
                                            <TableCell align="left" sx={{ color: 'blue', textDecoration: 'underline' }}>
                                                <label onClick={() => OpenPolicyFile(row)}>{row?.policyNumber}</label>
                                            </TableCell>
                                            <TableCell align="left">{row?.customerName}</TableCell>
                                            <TableCell align="left">{row?.registrationNumber}</TableCell>
                                            <TableCell align="left">{row?.paymentMode}</TableCell>
                                            <TableCell align="left">{row?.branchPayable?.Total}</TableCell>
                                            <TableCell align="left">{row?.userId?.email}</TableCell>
                                            <TableCell align="center">
                                                <Box className="ActionIcons d-flex" gap={2}>
                                                    {
                                                        row.isBranchPayable ?
                                                            <Tooltip title="edit" onClick={() => ApproveFunction(row)}>
                                                                <EditIcon />
                                                            </Tooltip> :
                                                            <Button
                                                                variant="outlined"
            ...
            >  {
                    openDrawer ? <CommonPayableDrawer
                        ...
                        BranchPayablePercentage={payablePercentage}
                    /> : null
                }
            </Drawer>
        </>
